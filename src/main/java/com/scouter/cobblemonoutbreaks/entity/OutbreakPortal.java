...
public class OutbreakPortal {
        ...
        for (int i = 0; i < spawnCount; i++) {
            ...
            if(allowSpawn) {
                ...
                if (CobblemonOutbreaksConfig.OUTBREAK_PORTAL_SPAWN_SOUND) {
                    float volume = CobblemonOutbreaksConfig.OUTBREAK_PORTAL_POKEMON_SPAWN_VOLUME;
    ...
    public List<ItemStack> spawnRewards(ServerLevel level, OutbreakPortalEntity gate) {
        boolean allowRewards = true;
        List<ItemStack> stacks = new ArrayList<>();
        gate.getOutbreakPortal().getOutbreakRewards().getRewards().forEach(r -> stacks.add(new ItemStack(r)));

        allowRewards = CobblemonOutbreaksEvent.Events.SPAWN_REWARDS.invoker().onSpawnRewards(level, stacks, allowRewards);
        if(allowRewards){
